...
bool is_not_empty(const char *line)
{
    if (!line)
        return false;
    while (*line != '\0')
    ...
    for (int i = 0; i < file_count; i++)
    {
        ...
        else
        {
            ...
            while (pos > 0)
            {
                ...
                if (read(fd, chunk, read_size) != (ssize_t)read_size)
                    break;
                ...
                for (int j = read_size - 1; j >= 0; j--)
                {
                    if (chunk[j] == '\n')
                    {
                        ...
                        char *full_line = malloc(full_len + 1);
                        if (seg_len > 0)
                            memcpy(full_line, chunk + j + 1, seg_len);
                        if (leftover_len > 0)
                            memcpy(full_line + seg_len, leftover, leftover_len);
                        full_line[full_len] = '\0';

                        if (is_not_empty(full_line))
                        {
                            if (number_lines)
                            {
                                printf("%d %s\n", current_line_num--, full_line);
                            }
                            else
                            {
                                printf("%s\n", full_line);
                ...
                if (segment_end > 0)
                {
                    ...
                    if (leftover_len > 0)
                        memcpy(new_left + segment_end, leftover, leftover_len);
            ...
            if (leftover_len > 0)
            {
                char *full_line = malloc(leftover_len + 1);
                memcpy(full_line, leftover, leftover_len);
                full_line[leftover_len] = '\0';
